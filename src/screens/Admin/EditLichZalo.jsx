...
export default function EditLichZalo() {
  ...
  useEffect(() => {
    if (task) {
      ...
      if (task.chuyenVienName && task.chuyenVienName.length > 0) {
        setUserSelections(task.chuyenVienName);
      }
    }
  }, [task]);
  ...
  const AddZaloAdmin = async () => {
    ...
    userSelections.forEach((item, index) => {
      ...
      data.append(`ToUser[${index}][idZaloToUser]`, item.idZaloToUser);
      data.append(`ToUser[${index}][zaloOrderID]`, item.zaloOrderID);
    });
    data.append('IDZaloOrder', task.idZaloOrder);
    data.append('UserCreateID', task.userCreateID);
    ...
    try {
      ...
      if (response.data.code == 200) {
        ...
        Toast.show({
          type: 'success',
          text1: `Sửa lịch thành công cho khách ${CustomerName}`,
          visibilityTime: 3000,
        });
      } else {
        setLoading(false);
    ...
    axiosInstance
      ...
      .then(response => {
        if (response.data && response.data.data) {
          ...
          if (task && task.userForMakeup) {
            const selectedUser = userOptions.find(
              user => user.value === task.userForMakeup,
            );
            if (selectedUser) {
              setUserForMakeup({
  ...
  const removeUserSelection = async index => {
    const itemToRemove = userSelections[index];
    if (itemToRemove.idZaloToUser == 0) {
      setUserSelections(prevSelections => {
      ...
      try {
        const data = new FormData();
        data.append('id', itemToRemove.idZaloToUser);
        const response = await axiosInstance.post(
          '/delete-user-make-order',
          data,
          {
            headers: {
              'Content-Type': 'multipart/form-data',
        ...
        console.log('Đã có lỗi vui lòng thử lại');
      }
    }
  };
